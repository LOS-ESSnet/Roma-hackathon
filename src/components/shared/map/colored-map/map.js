...
import { mapGlConfig } from 'config';
  ...
  render() {
    ...
    return (
      <MapGL
        ...
        mapboxApiAccessToken={mapGlConfig.TOKEN}
      >
        {pointCoords && pointCoords.length > 0 && (
          <Points pointCoords={pointCoords} pointContentArray={pointContentArray} />
        )}
        {legend && <Legend legend={legend} classes={classes} />}
        {contentArray.length !== 0 && hoveredFeature && (
          <Tooltip hoveredFeature={hoveredFeature} x={x} y={y} contentArray={contentArray} />
        )}
      </MapGL>
    );
  }
